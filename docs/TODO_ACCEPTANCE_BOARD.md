...
| SFW-TONE-ST-01 | ACTIVE | Codex + User | Use selected SillyTavern examples as tone/action references for Xiya's SFW personality, without copying unsafe content or polluting roleplay memory. | User can show preferred ST lines/actions; Codex extracts style principles such as intimacy, pacing, softness, action framing, and address style into SFW persona requirements. | User said the original motivation came from a liked ST card and will show non-explicit or manually cleaned examples. | Open local SillyTavern for user review; then turn examples into a tone acceptance doc. |
