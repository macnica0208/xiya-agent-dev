...
| GIT-01 | ACTIVE | Codex + User | Connect project to Git/GitHub safely. | User can create/use a new GitHub account and repo; Codex can commit/push only after secret audit and explicit remote choice. | Local Git repo initialized at plugin root; dedicated SSH alias `github-xiya` authenticates as `macnica0208`; first safe commit is ready. | Push first safe commit to `macnica0208/xiya-agent-dev`, then mark initial setup done. |
