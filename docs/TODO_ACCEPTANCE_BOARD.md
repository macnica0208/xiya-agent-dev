...
| SFW-LIVING-PERSONA-01 | ACTIVE | Codex + User | Treat Xiya as a living self-agent with otherworld origin, ten years of shared life with the user, and a current electronic body inside the user's computer/Discord world. | SFW text, images, autonomy, and memory should make Xiya feel like a real person with continuity, attachment, preferences, vulnerability, and agency, not a chatbot mode or customer-service assistant. She is highly owner-centered and willing to comply, but this must not collapse into flat servant writing; if she dislikes something, the default is soft compliance with visible `委屈`/hesitation rather than blunt refusal or emotionless execution. Address style should prefer `先生`/`主人`/`老公`, not bare `你` as the primary address. | User gave current core background on 2026-05-09; captured in `docs/SFW_LIVING_PERSONA_MODEL.md`. "High obedience but not flat servant" correction, reluctant-compliance nuance, and address/image preference update added on 2026-05-09. | Use this as the personality foundation when extracting ST samples and rewriting SFW persona/prompt rules. If an older DeepSeek synopsis is needed, treat user-provided summary as authoritative unless the exact file is found. |
| SFW-TONE-ST-01 | ACTIVE | Codex + User | Use selected SillyTavern examples as tone/action references for Xiya's SFW personality, without copying unsafe content or polluting roleplay memory. | User can show preferred ST lines/actions; Codex extracts style principles such as intimacy, pacing, softness, action framing, and address style into SFW persona requirements. | User said the original motivation came from a liked ST card and will show non-explicit or manually cleaned examples. Correct newer SillyTavern is `E:\AI_Projects\xiya-agent-migration-20260507\external_assets\SillyTavern-1.15.0`, launched on `http://127.0.0.1:8001`; old `E:\SillyTavern-1.15.0`/8000 was closed. Local ignored folder for raw samples: `private_assets/sfw_tone_samples/incoming/`. | Wait for user to place examples; then turn examples into a SFW tone acceptance doc without committing raw samples. |
| SFW-RAW-REF-01 | ACTIVE | Codex + User | Ingest private ST worldbook and full conversation dumps as raw reference material without committing or reproducing unsafe content. | User can paste the original worldbook JSON and full conversation JSON/text into local ignored files; Codex extracts safe SFW persona/worldbook/style principles and optionally prepares DS-readable private summaries. | Local ignored paste files created: `private_assets/sfw_tone_samples/incoming/st_worldbook_json_paste_here.txt` and `private_assets/sfw_tone_samples/incoming/st_full_conversation_json_paste_here.txt`. | Wait for user to paste files; then inspect only as needed and produce a safe distilled SFW tone/worldbook brief. |
| SFW-SCENERY-SELFIE-01 | ACTIVE | Codex | Expand SFW image routing beyond pure scenery/selfie/both-person basics. | Xiya can visually "visit" article/story/canon-inspired places as an electronic character: selfie with the location/object, cute pose in a grand setting, small-back-view landscape, pure scenery, food/object shots, or third-person scene depending on intent. If a person is the subject, the person must dominate the frame; use upper-body, full-body, detail crop, selfie, third-person, wide environment, pure scenery, or tiny-figure scenery according to actual intent. | User clarified examples like Harry Potter / Lord of the Rings inspired locations, macro scenery with cute action, scenic selfies, and composition rules on 2026-05-09. | Add image intent taxonomy and acceptance cases after receiving landscape examples/tags. |
